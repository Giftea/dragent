...
exports.generateAllocationReason = generateAllocationReason;
exports.analyzeAllocation = analyzeAllocation;
const sdk_1 = __importDefault(require("@anthropic-ai/sdk"));
const protocolMonitor_1 = require("./protocolMonitor");
const anthropic = new sdk_1.default({ apiKey: process.env.ANTHROPIC_API_KEY });
async function generateAllocationReason(best, alternatives) {
...
async function analyzeAllocation(maxRisk = "medium", preferredAsset = "USDC") {
    console.log("📊 Fetching DeFi protocol yields...");
    const yields = await (0, protocolMonitor_1.fetchProtocolYields)();
    if (yields.length === 0) {
    ...
    yields.slice(0, 5).forEach(y => {
        console.log(`   ${y.protocol} (${y.chain}): ${y.apy}% APY, ${y.risk} risk, $${(y.tvl / 1e6).toFixed(0)}M TVL`);
    });
    const best = (0, protocolMonitor_1.findBestYield)(yields, maxRisk, preferredAsset);
    if (!best) {
