...
const GASLESS_API    = process.env.GASLESS_API || "https://gasless.gokite.ai";
...
export async function gaslessTransfer(
  ...
  if (balance < value) {
    throw new Error(
      `Insufficient PYUSD balance. Have: ${ethers.formatUnits(balance, PYUSD_DECIMALS)}, need: ${amount}`
    );
  }
