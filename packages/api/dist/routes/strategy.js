...
router.post("/parse", (0, x402_1.requirePayment)("1000000000000000000", "Strategy parsing — powered by Claude AI", "/api/strategy/parse"), async (req, res) => {
    try {
        const { strategy } = req.body;
        if (!strategy)
            return res.status(400).json({ error: "strategy required" });
...
router.post("/reason/generate", (0, x402_1.requirePayment)("500000000000000000", "Trade reason generation — powered by Claude AI", "/api/reason/generate"), async (req, res) => {
    try {
        const { asset, price, rsi, priceChange4h, trend, confidence, strategy, action, sizeUSDC, } = req.body;
        if (!asset || !price || !rsi || !action || !strategy) {
            return res.status(400).json({ error: "Missing required fields" });
        }
        let lastError;
...
Write ONE sentence starting with "${action === "BUY" ? "Buying" : "Selling"}". No preamble, just the sentence.`,
                        },
                    ],
                });
